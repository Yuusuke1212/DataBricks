...
class JvDataWorker(QRunnable):
    ...
    def __init__(self, jvlink, option: int, from_date: str, data_spec_list: list):
        ...
        try:
            ...
            jvlink_exe_path = config_manager.get_jvlink_exe_path()
            self.jvlink_dir = Path(jvlink_exe_path).parent.resolve()
        except Exception as e:
            logging.warning(f"JV-Linkパス取得エラー: {e}. デフォルトパスを使用します。")
            self.jvlink_dir = Path("JV-Link").resolve()
    ...
    def run(self):
        ...
        try:
            ...
            for i, data_spec in enumerate(self.data_spec_list):
                ...
                # データ取得処理
                read_count_str = self._safe_jv_open(data_spec, self.from_date, self.option)
                read_count = int(
    ...
    def _safe_jv_open(self, data_spec, from_date, option):
        ...
        try:
            ...
            logging.debug(f"ワーキングディレクトリを元に戻しました: {original_cwd}")
...
class JvLinkManager(QObject):
    ...
    def __init__(self):
        ...
        self.current_realtime_worker = None

        # JV-Link.exeのパスを設定から取得
        try:
            from ..services.settings_manager import get_config_manager
            config_manager = get_config_manager()
            self.jvlink_exe_path = config_manager.get_jvlink_exe_path()
            self.jvlink_dir = Path(self.jvlink_exe_path).parent.resolve()
            logging.info(f"JV-Linkディレクトリ: {self.jvlink_dir}")
        except Exception as e:
            logging.warning(f"JV-Linkパス取得エラー: {e}. デフォルトパスを使用します。")
            self.jvlink_exe_path = "JV-Link/JV-Link.exe"
            self.jvlink_dir = Path("JV-Link").resolve()
    ...
    def async_initialize(self):
        ...
        try:
            ...
            if self._current_state == JVLinkState.UNINITIALIZED:
                self._set_state(JVLinkState.READY)
                logging.info("JV-LinkステータスをREADYに設定しました")
    ...
    def _safe_jv_init(self, service_key=""):
        """JVInitを安全に実行（ワーキングディレクトリ変更付き）"""
        return self._with_jvlink_working_directory(self.jvlink.JVInit, service_key)
