...
class AppController(QObject):
    ...
    def __init__(self, main_window: MainWindow = None, parent=None):
        ...
        # 統一通知システムの初期化（新機能）
        if main_window:
            self.notification_manager = initialize_notification_manager(main_window)
        else:
    ...
    def initialize_app(self):
        ...
        try:
            ...
            # 初回起動時のウェルカムウィザード表示（新機能）
            self._check_and_show_welcome_wizard()
    ...
    def _check_and_show_welcome_wizard(self):
        """
        初回起動時のウェルカムウィザード表示判定と実行（新機能）
        """
        try:
            from ..views.welcome_wizard import WelcomeWizard
            ...
            if WelcomeWizard.should_show_wizard(self.settings_manager):
                ...
                if result == wizard.Accepted:
                    ...
                    self.emit_log("INFO", "ウェルカムウィザードがキャンセルされました。")
            else:
                self.emit_log("INFO", "初回セットアップは完了済みです。")

        except Exception as e:
            self.emit_log("ERROR", f"ウェルカムウィザード表示エラー: {e}")

    def _on_welcome_wizard_completed(self):
        """
        ウェルカムウィザード完了時の処理（新機能）
        """
        try:
            from ..views.welcome_wizard import WelcomeWizard
            ...
            # 必要に応じて初期データの取得を促すメッセージを表示
            self._show_initial_data_suggestion()

        except Exception as e:
            self.emit_log("ERROR", f"ウェルカムウィザード完了処理エラー: {e}")

    def _show_initial_data_suggestion(self):
        """
        初回セットアップ完了後、初期データ取得を促すメッセージを表示（新機能）
        """
        try:
            from PySide6.QtWidgets import QMessageBox

            msg = QMessageBox(self.main_window)
            msg.setWindowTitle("次のステップ")
            msg.setIcon(QMessageBox.Information)
            msg.setText("セットアップが完了しました！")
            msg.setInformativeText(
                "競馬データの取得を開始しますか？\n\n"
                "「はい」をクリックすると、一括データ取得ダイアログが開きます。\n"
                "後で手動で実行することもできます。"
            )
            ...
            self.emit_log("ERROR", f"初期データ取得提案表示エラー: {e}")
    ...
    def initialize_connections(self):
        ...
        if hasattr(self.main_window.etl_setting_view, 'data_retrieval_requested'):
            ...
            if hasattr(self, 'start_data_acquisition_with_types'):
                ...
                self.main_window.etl_setting_view.data_retrieval_requested.connect(
                    self.start_setup_data_acquisition_with_types)
            else:
    ...
    def on_database_config_updated(self, db_config: dict):
        ...
        try:
            ...
            # 統一通知システムでエラー表示（改良版）
            self._show_error_message("データベース設定更新に失敗しました", str(e))
            # エラー発生時もUIを更新して状態を反映
            self._update_dashboard_db_info()
    ...
    def on_error_occurred(self, error_message: str):
        ...
        # 統一通知システムでエラー表示（改良版）
        self._show_error_message("JV-Linkでエラーが発生しました", error_message)
    ...
    def on_operation_finished(self, last_timestamp: str):
        ...
        try:
            ...
            # 統一通知システムで完了メッセージ表示（改良版）
            if last_timestamp == "キャンセル":
                self._show_warning_message("データ取得がキャンセルされました")
            else:
                self._show_success_message("データ取得が完了しました")

        except Exception as e:
            logging.error(f"データ取得完了処理中にエラー: {e}")
            # 統一通知システムでエラー表示
            self._show_error_message("完了処理でエラーが発生しました", str(e))
    ...
    def on_export_error(self, message: str):
        ...
        # 統一通知システムでエラー表示
        self._show_error_message("エクスポートでエラーが発生しました", message)
    ...
    def start_realtime_watch(self):
        ...
        if not self.jvlink_manager.is_initialized():
            ...
            # 統一通知システムでエラー表示
            self._show_error_message("JV-Link未初期化", error_msg)
        ...
        try:
            ...
            if not summary or all(info.get('count', 0) ==
                ...
                warning_msg = "データベースにデータが存在しません。まずセットアップまたは差分データを取得してください。"
                logging.warning(warning_msg)
                # 統一通知システムで警告表示
                self._show_warning_message("データベースが空です", warning_msg)
        ...
        try:
            ...
            # 統一通知システムで情報表示
            self._show_info_message("速報受信を開始しました")
        except Exception as e:
            logging.error(f"速報監視開始エラー: {e}")
            # 統一通知システムでエラー表示
            self._show_error_message("速報監視開始でエラーが発生しました", str(e))
        ...
        try:
            ...
            if 'database' in settings:
                ...
                if success:
                    ...
                    # 統一通知システムでエラー表示とダイアログ
                    self._show_error_message("データベース再接続に失敗しました", message, show_dialog=True)
            ...
            # 統一通知システムで成功メッセージ表示
            self._show_success_message("設定を保存しました")

        except Exception as e:
            logging.error(f"設定の保存中にエラーが発生: {e}")
            # 統一通知システムでエラー表示とダイアログ
            self._show_error_message("設定の保存でエラーが発生しました", str(e), show_dialog=True)
    ...
    # === アクティブデータベースプロファイル管理（新機能）===

    def _initialize_database_profiles(self):
        """
        アプリケーション起動時にデータベースプロファイルを初期化
        """
        try:
            # 設定からプロファイル一覧を取得
            profiles = self.settings_manager.get_database_profiles()
            active_profile = self.settings_manager.get_active_database_profile()

            # ダッシュボードにプロファイル一覧を設定
            if hasattr(self.main_window, 'dashboard_view'):
                self.main_window.dashboard_view.set_database_profiles(profiles, active_profile)

            self.emit_log("INFO", f"データベースプロファイル初期化完了: {len(profiles)}個、アクティブ: {active_profile}")

        except Exception as e:
            self.emit_log("ERROR", f"データベースプロファイル初期化エラー: {e}")
    ...
    def create_new_database_profile(self, profile_name: str, db_config: Dict[str, Any]) -> bool:
        ...
        try:
            # プロファイルを保存
            self.settings_manager.save_database_profile(profile_name, db_config)

            # ダッシュボードのプロファイル一覧を更新
            self._refresh_database_profiles()

            self.emit_log("INFO", f"新しいデータベースプロファイル '{profile_name}' を作成しました")
            return True

        except Exception as e:
            self.emit_log("ERROR", f"データベースプロファイル作成エラー: {e}")
            return False
    ...
    def delete_database_profile(self, profile_name: str) -> bool:
        ...
        try:
            ...
            # 現在アクティブなプロファイルを削除しようとしている場合
            current_active = self.settings_manager.get_active_database_profile()
            if current_active == profile_name:
                # デフォルトプロファイルに切り替え
                self.settings_manager.set_active_database_profile('default')

            # プロファイルを削除
            self.settings_manager.delete_database_profile(profile_name)

            # ダッシュボードのプロファイル一覧を更新
            self._refresh_database_profiles()

            self.emit_log("INFO", f"データベースプロファイル '{profile_name}' を削除しました")
            return True

        except Exception as e:
            self.emit_log("ERROR", f"データベースプロファイル削除エラー: {e}")
            return False

    def _refresh_database_profiles(self):
        """
        ダッシュボードのデータベースプロファイル一覧を更新
        """
        try:
            profiles = self.settings_manager.get_database_profiles()
            active_profile = self.settings_manager.get_active_database_profile()
            ...
            self.emit_log("ERROR", f"データベースプロファイル一覧更新エラー: {e}")

    def get_current_database_info(self) -> Dict[str, Any]:
        """
        現在のデータベース情報を取得
        ...
        try:
            active_profile = self.settings_manager.get_active_database_profile()
            db_config = self.settings_manager.get_database_profile_config(active_profile)

            return {
                'active_profile': active_profile,
                'config': db_config,
                'connected': self.db_manager and self.db_manager.engine is not None,
                'type': self.db_manager.get_db_type() if self.db_manager else 'Unknown',
                'name': self.db_manager.get_db_name() if self.db_manager else 'Unknown'
            }

        except Exception as e:
            self.emit_log("ERROR", f"データベース情報取得エラー: {e}")
            return {
    ...
    def _show_success_message(self, message: str, timeout: int = 5000):
        """
        統一通知システムを使用したステータスメッセージ表示（改良版）
    ...
    def _show_warning_message(self, message: str, timeout: int = 5000):
        """
        統一通知システムを使用した警告メッセージ表示（改良版）
    ...
    def _show_error_message(self, message: str, details: str = "", timeout: int = 5000, show_dialog: bool = False):
        """
        統一通知システムを使用したエラーメッセージ表示（改良版）
        
        Args:
            message: 主要メッセージ
            details: 詳細情報
            timeout: 表示時間（ミリ秒）
            show_dialog: ダイアログ表示を行うかどうか
        """
        # 旧来のステータスバー更新も維持（互換性のため）
        if hasattr(self.main_window, 'statusBar'):
            self.main_window.statusBar().showMessage(message, timeout)
        ...
        if show_dialog:
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(self.main_window, "エラー", message)

    def _show_info_message(self, message: str, details: str = "", timeout: int = 4000):
        """
        情報メッセージを統一通知システムで表示（新機能）
    ...
    def on_active_database_changed(self, db_profile_name: str):
        """アクティブデータベースの変更処理（将来実装予定）"""
        try:
            logging.info(f"アクティブデータベース変更要求: {db_profile_name}")
            # TODO: データベースプロファイル切り替え機能の実装
            logging.warning("データベースプロファイル切り替え機能は未実装です")
        except Exception as e:
