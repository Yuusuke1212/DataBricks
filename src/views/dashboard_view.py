...
class DashboardView(CardWidget):
    ...
    修正点: アクティブデータベース概念の導入とプルダウンメニュー統合
    """
    ...
    # 新しいシグナル: アクティブデータベース変更
    active_database_changed = Signal(str)
    ...
    def __init__(self, parent=None):
        ...
        # 修正点4: 強化されたデータベース接続情報カードを追加
        self.db_info_card = self._create_enhanced_db_info_card()
    ...
    def _create_enhanced_db_info_card(self) -> CardWidget:
        """
        修正点4: 強化されたデータベース接続情報表示カード（アクティブデータベース概念付き）
        """
        db_card = CardWidget(self)
        db_card.setFixedWidth(300)

        # カードタイトル
        db_card_title = StrongBodyLabel("🗄️ アクティブデータベース", db_card)
        ...
        profile_layout.addWidget(BodyLabel("プロファイル:"))
        self.db_profile_combo = ComboBox(db_card)
        self.db_profile_combo.setMinimumWidth(120)
        self.db_profile_combo.currentTextChanged.connect(self._on_profile_changed)
    ...
    def _on_profile_changed(self, profile_name: str):
        """データベースプロファイル変更時の処理"""
        if profile_name != self.current_active_profile:
            self.current_active_profile = profile_name
            self.active_database_changed.emit(profile_name)

    def set_database_profiles(self, profiles: dict[str, dict[str, any]], active_profile: str = 'default'):
        """
        データベースプロファイル一覧を設定し、プルダウンメニューを更新
        
        Args:
            profiles: プロファイル名をキーとするデータベース設定の辞書
            active_profile: 現在アクティブなプロファイル名
        """
        self.database_profiles = profiles
        self.current_active_profile = active_profile

        # コンボボックスを更新
        self.db_profile_combo.clear()
        for profile_name in profiles.keys():
            display_name = "デフォルト" if profile_name == 'default' else profile_name
            self.db_profile_combo.addItem(display_name, profile_name)

        # アクティブプロファイルを選択
        for i in range(self.db_profile_combo.count()):
            if self.db_profile_combo.itemData(i) == active_profile:
                self.db_profile_combo.setCurrentIndex(i)
                break

    def get_current_active_profile(self) -> str:
        """現在選択されているプロファイル名を取得"""
        current_index = self.db_profile_combo.currentIndex()
        if current_index >= 0:
            return self.db_profile_combo.itemData(current_index)
        return 'default'
